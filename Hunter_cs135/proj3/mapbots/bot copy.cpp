...
Action stepFromTo(Loc fromLoc, Loc toLoc) {
  Action nextStep = COLLECT;
  if (fromLoc.r > toLoc.r) nextStep = UP;
  else if (fromLoc.r < toLoc.r ) nextStep = DOWN;
  else if (fromLoc.c > toLoc.c ) nextStep = LEFT;
  else if (fromLoc.c < toLoc.c ) nextStep = RIGHT;
  return(nextStep);
}
Action randomMove() {
