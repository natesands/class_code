...
int findNearestBotAny(Loc loc,Area area);
int findNearestBotNoDebris(Loc loc,Area area);
             ...
             Area &area, ostream &log)
{
	...
	COLS = cols;
  MPR = mpr;
  //if (mpr == .4 && COLS == 15 && ROWS == 15 && NUM == 30) {
   // CRITICAL_CAPACITY = NUM * (.2);
  //}
 // else 
 // if (mpr == .4) {
  //  CRITICAL_CAPACITY = NUM * (.55);
 // }
 // else if (mpr == .02) {
  //  CRITICAL_CAPACITY = NUM * (1);
 // }
  //else {
...
//if bot is assigned repair job
  ...
  if (map[bot[id].job.loc.r][bot[id].job.loc.c].hasBot &&
      loc.r != bot[id].job.loc.r && loc.c != bot[id].job.loc.c) {
      unassign(id);
  }
  ...
  if (area.inspect(row,col) == DEBRIS) {
    //if debris is not assigned
    if (!map[loc.r][loc.c].debrisAssigned) {  
      unassign(id);
      assignDebris(id, loc);
      return COLLECT;
    }
    //bot is at debris site assigned
    else if (loc.r == bot[id].job.loc.r && loc.c == bot[id].job.loc.c) {
      unassign(id);
      return COLLECT;
    }
    //bot is at debris site assigned to another bot 
    else if (map[loc.r][loc.c].botAssigned != id) {
      unassign(map[row][col].botAssigned); 
      unassign(id);
      assignDebris(id,loc); 
      return COLLECT;
    }   
  ...
  //if (NUM_BROKEN_BOTS > (int) CRITICAL_CAPACITY) return REPAIR_UP;
   // if (NUM_BROKEN_BOTS > (int) CRITICAL_CAPACITY) {
    //  if (bot[id].job.type == COLLECT_DEBRIS) return fastMoveFromTo(loc,bot[id].job.loc);
 //   } 
  ...
  if (UNASSIGNED_DEBRIS > 0) {
    ...
    log << id << " assigned debris (" << debrisLoc.r << " " << debrisLoc.c << ")\n";
    return moveFromTo(loc,bot[id].job.loc);
   }
...
void onRobotMalfunction(int id, Loc loc, Area &area, ostream &log) {
  ...
  int repairBot;
  if (REL_DENSITY > .025) 
    repairBot = findNearestBotAny(loc,area);
  else repairBot = findNearestBotNoDebris(loc,area); 
...
//calculates debris density for each map square
void refreshMap(Area area,ostream &log) {
  UNASSIGNED_DEBRIS = 0;
  NUM_DEBRIS=0;
 ...
 // Loc mapLoc;
  ...
  for (int r = 0; r < ROWS; r++) {
    for (int c = 0; c < COLS; c++) {
      map[r][c].hasBot = false;
      map[r][c].hasBrokenBot = false;
      if (area.inspect(r,c) != DEBRIS) map[r][c].debrisAssigned = false;
  ...
  for (int b = 0; b < NUM; b++) {
    ...
    if (bot[b].isBroken) {
      ...
      if (map[area.locate(b).r][area.locate(b).c].repairBotAssigned == false) {
        UNASSIGNED_BROKEN_BOTS++;
      }
    }
  }
  REL_DENSITY = (double) NUM_DEBRIS / (ROWS * COLS);
}
...
Action nextRepairAction(int id,Loc botLoc) {
  ...
  if (distance(botLoc,bot[id].job.loc) > 1) {
    if (MPR == .4) return moveFromTo(botLoc,bot[id].job.loc);
    else return fastMoveFromTo(botLoc,bot[id].job.loc);
  }
...
void assignDebris(int id, Loc jobLoc) {
      ...
      map[jobLoc.r][jobLoc.c].botAssigned = id;
}
...
Loc nearestDebris(Loc loc, Area area) {
  ...
  for (int r = 0; r < ROWS; r++) {
    for (int c = 0; c < COLS; c++) {
      ...
      if (area.inspect(r,c) == DEBRIS && !map[r][c].hasBot) {
        if (dist < minDist) {
          minDist = dist;
          nearest = mapLoc;
        }
...
int findNearestBotNoDebris(Loc loc,Area area) {
  ...
  for (int id = 0; id < NUM; id++) {
    ...
    if (dist < minDist && 
          !bot[id].isBroken &&
            area.inspect(botLoc.r,botLoc.c)!= DEBRIS) {
      minDist = dist;
      nearestBot = id;
    }
  }
