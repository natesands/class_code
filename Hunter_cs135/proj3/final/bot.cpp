...
//if bot is assigned repair job
  ...
  //if bot assigned to collect debris but there is bot at site already, unassign 
  if (bot[id].job.type == COLLECT_DEBRIS && map[bot[id].job.loc.r][bot[id].job.loc.c].hasBot &&
...
//  if (NUM_BROKEN_BOTS > (int) CRITICAL_CAPACITY) {
 ...
 // } 
  ...
  if (UNASSIGNED_DEBRIS > 0) {
    ...
    if (debrisLoc.r != NONE) assignDebris(id,debrisLoc);
