...
UNASSIGNED JobType should not be needed since each bot
has hasJob flag
...
enum JobType {FIND_DEBRIS,REPAIR_BOT,RANDOM_WALK,UNASSIGNED};
/* STRUCTS */
...
void assignDebrisJob(int id, Loc botloc, Area area);
...
Action onRobotAction(int id, Loc loc, Area &area, ostream &log) {
	...
	int col = loc.c;
  //assert: if bot is available it must
  //not be broken 
  bot[id].isBroken = false;
  ...
  //if bot was assigned repair job, check if target bot
  //is still broken.  if not, unsassign job
  if (bot[id].job.type = REPAIR_BOT) {
    if (!repairJobActive(id)) unassign(id);
  }
  //check if there are any unassigned repair jobs
  //if bot is not currently assigned to repair job,
  //unassign current job and re-assign assign
  //repair

  if (UNASSIGNED_BROKEN_BOTS > 0) {
    int brokenId;
    if (!bot[id].job.type == REPAIR_BOT) {
      brokenId = findBrokenBot();
      if (brokenID != NONE) { 
        unassign(id);

        bot[id].hasJob = true;
        bot[id].job.loc.r = area.locate(brokenId).r;
        bot[id].job.loc.c = area.locate(brokenId).c;
        bot[id].job.type = REPAIR_BOT;
        bot[id].job.targetBot = brokenId;
      }
    }
  }
...
void onRobotMalfunction(int id, Loc loc, Area &area, ostream &log) {
  ...
  //set bot's isBroken flag and assign repair crew
  bot[id].isBroken = true;
  assignRepair(id,loc,area);
}
...
void refreshMap(Area area) {
  ...
  //count number of bots that are broken
  //and are not assigned a repair crew
  for (int b = 0; b < NUM; b++) {
    map[area.locate(b).r][area.locate(b).c].hasBot = true;
    if (bot[b].isBroken && !repairBotAssigned) {
...
//checks if broken bot assigned to bot[id] is still broken
bool repairJobActive(int id) {
  int brokenBot;
  brokenBot = bot[id].job.targetBot;
  return (bot[brokenBot].isBroken);
}
//PROTOTYPE!
void assignRepair(int brokenBotId, Loc loc, Area area){
  int repairBot;
  repairBot = findNearestBot(loc, area);
  if (repairBot != NONE) {
    unassign(repairBot);
    bot[repairBot].hasJob = true;
    bot[repairBot].job.loc.r = loc.r;
    bot[repairBot].job.loc.c = loc.c;
    bot[repairBot].job.type = REPAIR_BOT;
    bot[repairBot].job.targetBot = brokenBotId;

    bot[brokenBotId].repairBotAssigned = true;
  }
  else {
    bot[brokenBotId].repairBotAssigned = false;
  }
  //if no bots are available, refresh map function
  //will detect unassigned broken and onrobot action
  //function will assign next available bot
}      
    
//PROTOTYPE
//find closest available bot not assigned to 
 //repair job
//returns closest bot ID or NONE 
int findNearestBot(Loc loc, Area area) {
} 


//PROTOTYPE
//scans map to find first broken bot
//without an assigned repair Bot
//should return NONE if no broken bot found

//finds the first bot that is broken
//and does not have a repair bot assigned
