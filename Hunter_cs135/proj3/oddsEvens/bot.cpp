...
Action onRobotAction(int id, Loc loc, Area &area, ostream &log) {
  ...
  //odds repair evens
  if (MPR == .4) {
    //odd man out gets a holiday
    if (NUM % 2 !=0 && id == (NUM - 1)) return randomMove(); 
    //what the odds do 
    if (id % 2 != 0) {
      Loc partnerBotLoc;
      partnerBotLoc.r = area.locate(id-1).r;
      partnerBotLoc.c = area.locate(id-1).c;
      //if partner bot is broken, fix 
      if (bot[id-1].isBroken) {
        return nextRepairAction(id,loc);
      } 
      else if (distance(loc,partnerBotLoc) == 1) {
        return randomMove();
      }
      else return fastMoveFromTo(loc,partnerBotLoc);
    }
    //evens do the collecting
    if (id % 2 == 0) {
